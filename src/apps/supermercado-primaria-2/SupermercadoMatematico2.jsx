...
const SupermercadoMatematico2 = () => {
    ...
    const generarNuevaMision = () => {
        ...
        // 50% de probabilidad de que sea un problema de suma o de resta (calcular el cambio)
        const esProblemaDeSuma = Math.random() > 0.5;

        const numProductos = Math.floor(Math.random() * 2) + 2; // Entre 2 y 3 productos
        ...
        for (let i = 0; i < numProductos; i++) {
            ...
            const cantidad = Math.floor(Math.random() * 2) + 1; // Entre 1 y 2 unidades
        ...
        if (esProblemaDeSuma) {
            ...
            const billetes = [5, 10];
            const billeteUsado = billetes.find(b => b > totalCompra) || 10;
    ...
    const comprobarRespuesta = () => {
        const respuestaUsuario = parseFloat(respuesta.replace(',', '.')); // Aceptamos comas y puntos
    ...
    return (
        <div id="supermercado-app-container">
            ...
            <div className="respuesta-usuario">
                ...
                <input type="text" value={respuesta} onChange={(e) => setRespuesta(e.target.value)} placeholder="Escribe el total"/>
