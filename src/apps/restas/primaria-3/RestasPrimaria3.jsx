...
const RestasPrimaria3 = ({ onGameComplete } = {}) => {
  ...
  const generarParTest = useCallback(() => {
    const digits = 3 + Math.floor(Math.random() * 2); // 3 o 4 cifras
    const min = Math.pow(10, digits - 1);
    const max = Math.pow(10, digits);
    let n1, n2;
    do {
            ...
            <React.Fragment key={i}>
              {i !== 0 && (
                <div
                  ...
                  style={{
                    gridRow: '1', gridColumn: String(circleCol),
              ...
              <div className="digit-display" style={{ gridRow: '1', gridColumn: String(digitCol) }}>{strNum1[i]}</div>
              <div
