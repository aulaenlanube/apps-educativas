  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogPrimitive.Portal>
        ...
        <DialogContent 
            className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-50 w-[90vw] max-w-[425px] bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-2xl border-2 border-pink-100 dark:border-slate-700"
        >
          <DialogHeader>
            <div className="mx-auto bg-pink-100 dark:bg-pink-900/40 w-16 h-16 rounded-full flex items-center justify-center mb-4">
                <Heart className="w-8 h-8 text-pink-600 fill-pink-600 animate-pulse" />
            </div>
            <DialogTitle className="text-2xl font-bold text-gray-800 dark:text-gray-100">¿Te gusta esta App?</DialogTitle>
          </DialogHeader>
          ...
          <DialogPrimitive.Close className="absolute top-4 right-4 rounded-full p-2 text-gray-400 hover:bg-gray-100 dark:hover:bg-slate-700 hover:text-gray-600 dark:hover:text-gray-200 transition-all">
            <X className="h-5 w-5" />
