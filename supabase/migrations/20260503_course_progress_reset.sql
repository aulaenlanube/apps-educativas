-- =====================================================
-- COURSE PROGRESS RESET (soft-reset por par alumno-grupo)
-- =====================================================
-- Permite al docente reiniciar el progreso del CURSO de un alumno
-- (tareas, duelos del grupo, batallas, intentos contables) sin afectar
-- el progreso global (XP, nivel, avatares desbloqueados, insignias,
-- historial de partidas, tiempo de juego). El reset es por par
-- (student_id, group_id): si un alumno está en varios grupos, reiniciar
-- uno no afecta a los otros.
--
-- Mecanismo: marca de corte `progress_reset_at` en student_groups.
-- Las RPCs de cálculo de nota filtran por created_at > reset_at en las
-- fuentes que aportan a la nota. No se borra ningún registro.
-- =====================================================

-- 1) Columna de marca de corte en la pivote alumno-grupo
ALTER TABLE public.student_groups
  ADD COLUMN IF NOT EXISTS progress_reset_at TIMESTAMPTZ NULL;

COMMENT ON COLUMN public.student_groups.progress_reset_at IS
  'Si NOT NULL, el cálculo de nota del curso ignora todo dato anterior a este timestamp para ese par alumno-grupo. Datos globales (XP, avatares, insignias, historial) no se ven afectados.';

CREATE INDEX IF NOT EXISTS idx_student_groups_reset_at
  ON public.student_groups (student_id, group_id)
  WHERE progress_reset_at IS NOT NULL;

-- 2) Tabla de log con histórico completo de resets
CREATE TABLE IF NOT EXISTS public.student_group_reset_log (
...
CREATE INDEX IF NOT EXISTS idx_student_group_reset_log_lookup
  ON public.student_group_reset_log (student_id, group_id, reset_at DESC);

ALTER TABLE public.student_group_reset_log ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS student_group_reset_log_teacher_select ON public.student_group_reset_log;
CREATE POLICY student_group_reset_log_teacher_select
  ON public.student_group_reset_log
  FOR SELECT
  TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM public.groups g
      WHERE g.id = student_group_reset_log.group_id
        AND g.teacher_id = auth.uid()
    )
    OR EXISTS (
      SELECT 1 FROM public.group_teachers gt
      WHERE gt.group_id = student_group_reset_log.group_id
        AND gt.teacher_id = auth.uid()
    )
  );

-- 3) RPC: el docente reinicia el progreso del curso de un alumno suyo
CREATE OR REPLACE FUNCTION public.teacher_reset_student_course_progress(
  p_student_id uuid,
  p_group_id   uuid
)
RETURNS json
...
BEGIN
  ...
  -- Validar que el caller es docente del grupo (titular o co-docente)
  SELECT
    EXISTS (SELECT 1 FROM public.groups g WHERE g.id = p_group_id AND g.teacher_id = v_caller)
    OR EXISTS (
      SELECT 1 FROM public.group_teachers gt
      WHERE gt.group_id = p_group_id AND gt.teacher_id = v_caller
    )
  INTO v_authorized;
  ...
  -- Validar que el alumno pertenece al grupo
  IF NOT EXISTS (
    SELECT 1 FROM public.student_groups
    WHERE student_id = p_student_id AND group_id = p_group_id
  ) THEN
    RETURN json_build_object('success', false, 'error', 'student_not_in_group');
  END IF;

  -- Marca de corte
  UPDATE public.student_groups
  SET progress_reset_at = now()
  WHERE student_id = p_student_id AND group_id = p_group_id
  RETURNING progress_reset_at INTO v_reset_at;

  -- Log
  INSERT INTO public.student_group_reset_log (student_id, group_id, teacher_id, reset_at)
  VALUES (p_student_id, p_group_id, v_caller, v_reset_at);

  SELECT COUNT(*)::int INTO v_total_resets
  FROM public.student_group_reset_log
  WHERE student_id = p_student_id AND group_id = p_group_id;

  RETURN json_build_object(
    'success', true,
    'reset_at', v_reset_at,
    'total_resets', v_total_resets
  );
END;
$function$;

GRANT EXECUTE ON FUNCTION public.teacher_reset_student_course_progress(uuid, uuid) TO authenticated;

-- 4) Reescritura de _compute_student_term_grades:
--    - Carga progress_reset_at del par (student, current group)
--    - Filtra game_sessions, quiz_battle_sessions y duel_grade_ledger por created_at > reset_at
--    - Filtra duel_events por d.group_id = v_group_id (corrige sub-bug cross-grupo:
--      antes el bonus de duelos sumaba duelos de cualquier grupo del alumno)
--    - bonus de avatares y nivel siguen siendo globales (sin filtrar)
...
BEGIN
  ...
  -- Punto de corte para el cálculo de nota del curso (NULL = sin reset).
  ...
  battle_termed AS (
    -- Limitación conocida: quiz_battle_sessions no tiene group_id, así que no
    -- se puede atribuir 1:1 una batalla a un grupo concreto. Mejora futura:
    -- añadir group_id a quiz_battle_sessions. Mientras tanto, el filtro por
    -- v_reset_at acota correctamente al período post-reset.
    SELECT
      COALESCE(
        qbs.term,
        CASE
  ...
  duel_events AS (
    -- Solo duelos del grupo activo (corrige el comportamiento previo, que
    -- sumaba duelos de todos los grupos del alumno) y posteriores al reset.
    SELECT
      CASE
...
-- 5) student_get_grade_with_duel_bonus: filtra history y bonus_raw por
--    grupo activo + reset_at, para que el alumno no vea duelos previos al
--    reset ni de otros grupos en su panel "Mi nota actual".
...
BEGIN
  ...
  SELECT COALESCE(SUM(l.delta), 0) INTO v_duel_bonus_raw
  FROM public.duel_grade_ledger l
  JOIN public.duels d ON d.id = l.duel_id
  WHERE l.student_id = p_student_id
    AND d.group_id = v_group_id
