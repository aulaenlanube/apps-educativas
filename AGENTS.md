# AGENTS.md — Guía para trabajar en EduApps
...
**Importante — `unique_apps`, `subject_exams`, `perfect_exams`, `high_score_exams` cuentan APPS DISTINTAS** (no sesiones). La unidad de "app distinta" es la tupla `(app_id, level, grade, subject_id)`: la misma app jugada en cursos o asignaturas diferentes cuenta como apps separadas (ej. Rosco de 1ºPrim Lengua y Rosco de 2ºPrim Inglés = 2). Esto evita que un alumno spamee la misma variante y obliga a recorrer la plataforma. **No se puede usar sólo `app_id`**: hay apps (busca-el-intruso, ordena-la-frase, rosco-del-saber, ahorcado, millonario, …) que comparten un único `app_id` para todos los cursos; con `COUNT(DISTINCT app_id)` ningún avatar con `count > ~50` sería alcanzable porque hay menos de ese número de `app_id` únicos en el catálogo. `count` en `unlock_requirement` es el nº de tuplas distintas requeridas. `app_sessions` (atado a un `app_id`) sí cuenta sesiones — se reserva para "completa N veces ESTA app" con counts modestos (≤5).
